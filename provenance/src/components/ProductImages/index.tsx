import { useState, useRef, RefObject, MutableRefObject, Key, useEffect } from "react";
  ...
  return (
    <div className="border-palette-lighter w-full max-w-md rounded border bg-white shadow-lg md:w-1/2">
      <div className="relative h-96">
        <Image
          src={mainImg.replace("ipfs://", "https://ipfs.io/ipfs/")}
          alt={mainImg}
          layout="fill"
      ...
      <div className="border-palette-lighter relative flex border-t">
        ...
        >
          {images.map((imgItem: string, index: Key) => (
            <button
              key={index}
              className="relative h-32 w-40 flex-shrink-0 rounded-sm "
              onClick={() =>
                setMainImg(imgItem.replace("ipfs://", "https://ipfs.io/ipfs/"))
              }
            >
              <Image
                src={imgItem.replace("ipfs://", "https://ipfs.io/ipfs/")}
                alt={imgItem}
                layout="fill"
                className=""
              />
